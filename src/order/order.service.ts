...
export class OrderService {
  ...
  async createOrder(createOrderDto: CreateOrderDto) {
    ...
    for (const orderItem of orderDetails) {
      ...
      if (!product || !product.isAvailable || product.stock < orderItem.quantity) {
        throw new BadRequestException(`Product ${orderItem.productId} is not available or out of stock`);
      }
  ...
  async deleteOrder(id: string) {
    try {
      ...
      if (timeDifferenceInMinutes > 10) {
        throw new ForbiddenException("Order cannot be deleted after 10 minutes of creation");
      }
      ...
      throw new InternalServerErrorException(error.message || "Cannot delete order");
    }
  }
